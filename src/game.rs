use piston_window::*;
use piston_window::types::Color;

use rand::{ thread_rng, Rng };

use crate::snake::{Direction, Snake};
use crate::draw::{draw_pixel, draw_rect};

const FOOD_COLOR: Color = [ 0.8, 0.0, 0.0, 1.0];
const GAME_OVER_COLOR: Color = [ 0.8, 0.0, 0.0, 0.2];

const FPS: f64 = 0.1;
const RESTART_TIME: f64 = 1.0;

pub struct Game {
	snake: Snake,
	
	food_exist: bool,
	food_x: i32,
	food_y: i32,

	width: i32,
	height: i32,

	game_over: bool,
	waiting_time: f64,
}

impl Game {
	pub fn new(width: i32, height: i32) -> Game {
		Game {
			snake: Snake::new(2, 2),
			waiting_time: 0.0,
			food_exist: true,
			food_x: 6,
			food_y: 4,
			width: width,
			height: height,
			game_over: false,
		}
	}

	pub fn key_pressed(&mut self, key: Key) {
		if self.game_over {
			return;
		}

		let direction = match key {
			Key::Up => Some(Direction::Up),
			Key::Down => Some(Direction::Down),
			Key::Left => Some(Direction::Left),
			Key::Right => Some(Direction::Right),
			_ => None,
		};

		if direction.unwrap() == self.snake.head_direction().inverse() {
			return;
		}

		self.update_snake(direction);
	}

	pub fn draw(&self, context: &Context, graphic: &mut G2d) {
		self.snake.draw(context, graphic);

		if self.food_exist {
			draw_pixel(FOOD_COLOR, self.food_x, self.food_y, context, graphic);
		}

		if self.game_over {
			draw_rect(GAME_OVER_COLOR, 0, 0, self.width, self.height, context, graphic);
		}
	}

	pub fn update(&mut self, delta_time: f64) {
		self.waiting_time += delta_time;
		
		if self.game_over {
			if self.waiting_time > RESTART_TIME {
				self.restart();
			}
			return;
		}

		if !self.food_exist {
			self.add_food();
		}

		if self.waiting_time > FPS {
			self.update_snake(None);
		}
	}

	fn restart(&mut self) {
		self.snake = Snake::new(2, 2);
		self.game_over = false;
		self.food_exist = true;
		self.food_x = 6;
		self.food_y = 4;
		self.waiting_time = 0.0;
	}

	fn add_food(&mut self) {
		let mut range = thread_rng();
		let mut new_x = range.gen_range(0, self.width);
		let mut new_y = range.gen_range(0, self.height);

		while self.snake.overlap_tail(new_x, new_y) {
			new_x = range.gen_range(0, self.width);
			new_y = range.gen_range(0, self.height);
		}

		self.food_x = new_x;
		self.food_y = new_y;
		self.food_exist = true;
	}

	fn update_snake(&mut self, direction: Option<Direction>) {
		if self.check_snake_alive(direction) {
			self.snake.move_forword(direction);
			self.check_eating();
		} else {
			self.game_over = true;
		}
		self.waiting_time = 0.0;
	}

	fn check_eating(&mut self) {
		let (head_x, head_y) = self.snake.head_position();
		if self.food_exist && self.food_x == head_x && self.food_y == head_y {
			self.food_exist = false;
			self.snake.restore_tail();
		}
	}

	fn check_snake_alive(&self, direction: Option<Direction>) -> bool {
		let (next_x, next_y) = self.snake.next_head(direction);
		if self.snake.overlap_tail(next_x, next_y) {
			return false;
		}
		next_x >= 0 && next_y >= 0 && next_x < self.width && next_y < self.height
	}
}